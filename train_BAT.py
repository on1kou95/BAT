...
def train_classifier(run_id, restart, saved_model, linear):
    ...
    save_dir = os.path.join(cfg.weights, run_id)
    ...
    f_t = open('confusion_matrix.txt', 'w')
            ...
            if epoch in val_array:
                ...
                for val_iter in range(len(modes)):
                    try:
                        ...
                        predictions1 = np.zeros((len(list(pred_dict.keys())),4))
                        ground_truth1 = []
                        entry = 0
                        ...
                        print("c_pred1:",c_pred1)
                        print("ground_truth1:",ground_truth1)
                        print("correct_count1:",correct_count1)
                        print("accuracy11:",accuracy11)
                        f_t.write(str(c_pred1) + '\n')
                        f_t.write(str(accuracy11) + '\n')
                ...
                predictions = np.zeros((len(list(pred_dict.keys())),4))
                ground_truth = []
                entry = 0
                for key in pred_dict.keys():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--saved_model", dest='saved_model', type=str, required=False, default= "./data/Pretrained/BAT.pth",
                        help='run_id')
